  ...
  useEffect(() => {
    ...
    function getAllClusters() {
      return seedData.trees.reduce(
        (acc: any[], tree: any) => acc.concat(tree.clusters),
        [] as any[],
      );
    }
    ...
    function createClusterNode(
      ...
      const childClusters = allClusters.filter(
        (c) => c.parent_id === cluster.uid,
      );
      ...
      return {
        ...
        children: [
          ...childClusters.map((child) =>
            createClusterNode(child, allClusters, areas),
          ),
      ...
      return {
        ...
        children: data.trees.map((tree: any) => ({
          ...
          children: tree.clusters
            ...
            .map((c: any) => createClusterNode(c, tree.clusters, data.areas)),
        })),
      } as TreeNode;
    }
    ...
    function showNodeDetails(node: any) {
      ...
      content.className =
        "nf-card rounded-lg p-6 max-w-2xl w-full mx-4 max-h-96 overflow-y-auto";
      ...
      if (node.type === "tree") {
        ...
        const areas = seedData.areas.filter(
          (a) => a.cluster_uid === cluster.uid,
        );
        details = `
    ...
    const render = () => {
      ...
      node
        .append("circle")
        .attr("r", (d: any) =>
          d.data.type === "tree" ? 12 : d.data.type === "cluster" ? 8 : 6,
        )
        .style("fill", (d: any) => {
      ...
      node
        ...
        .style("font-size", (d: any) =>
          d.data.type === "tree" ? "14px" : "12px",
        )
        .style("font-weight", (d: any) =>
          d.data.type === "tree" ? "600" : "400",
        )
      ...
      node
        ...
        .attr("cy", -8)
        .style("fill", (d: any) =>
          d.data.status === "active" ? "var(--nf-success)" : "var(--nf-error)",
        )
